...
class TwitterHelper extends AppHelper {
     ...
     * 	style:		''
     */
    public function oauthLink($options = array(),$extras = array()) {
        ...
        $options = am($default, $options);
		$options = am($default_extras, $options);
        ...
        $this->Js->buffer("
            ...
            var link = $('<a id=\"".$options['id']."\" class=\"".$options['class']."\" >').attr('href', data.url).html('{$options['login']}');

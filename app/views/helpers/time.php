 ...
 */
	...
	function niceShort($dateString = null, $userOffset = null, $timeFormat = '%H:%M') {
		...
		if ($this->isToday($dateString, $userOffset)) {
			...
			$ret = sprintf(__('Today, %s',true), strftime($timeFormat, $date));
		} elseif ($this->wasYesterday($dateString, $userOffset)) {
			//$ret = sprintf(__('Yesterday, %s',true), strftime("%H:%M", $date));
			$ret = sprintf(__('Yesterday, %s',true), strftime($timeFormat, $date));
		} else {
			//$format = $this->convertSpecifiers("%b %eS{$y}, %H:%M", $date);
			$format = $this->convertSpecifiers("%b %eS{$y}, $timeFormat", $date);

...
if ($assetType == 'css') {
	if(USING_THEME){
		$filePath = App::themePath('default').'webroot/css'.DS.$regs[1];
	}else{
		$filePath = CSS . $regs[1];
	}
	$fileType = 'text/css';
	...
	if(USING_THEME){
		$filePath = App::themePath('default').'webroot/js'.DS.$regs[1];
	}else{
		$filePath = JS . $regs[1];
	}
...
function compress($path, $name, $type) {
	...
	if ($type == 'css') {
		...
		/*
			CHANGED Removed because of the following comment.
			http://milesj.me/blog/read/cakephp-asset-compression/page:2
		*/
		$stylesheet = preg_replace('/\s\s+/', ' ', $input);
